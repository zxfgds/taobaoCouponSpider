...
class GetItemApi:
    ...
    def getAppKey(self):
        ...
        cur = conn.cursor()
        sql = 'SELECT `key`, `secret`, `adzoneid` FROM z_ali_appkey  WHERE ban_time < %d  order by RAND() limit 1' % int(time.time())
        try:
            cur.execute(sql)
            r = cur.fetchone()
            return r
    ...
    def updateBantime(self,key,bantime):
        ...
        cur = conn.cursor ()
        sql = 'UPDATE z_ali_appkey SET ban_time = %d WHERE `key` = %s ' % (int(bantime) + int(time.time()) , key)
        try:
            cur.execute (sql)
            conn.commit()
        except Exception, e:
    ...
    def make_steps(self, keyword, catid, start_price, end_price, step_size, sleep_time):
        try:
